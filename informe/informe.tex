...
\textsc{\large (con anillos dinámicos)}\\[0.5cm] % Minor heading such as course title
\HRule \\[1cm]
...
Cada nodo va a estar permanentemente escuchando por mensajes que anuncien anillos nuevos. Los nodos prefieren estar con líderes con \emph{ID}s más grandes. \textbf{Si un nodo detecta un anillo cuyo líder posee un \emph{ID} más grande que el de el líder del anillo actual entonces abandona su anillo y procede a unirse al anillo detectado.}
...
Para que el nodo \emph{A} obtenga un lock sobre un archivo primero se fija en la lista de locks globales que posee. Si no aparece, entonces no está bloqueado y procede a intentar adquirirlo.


Para hacerlo envia un mensaje de anillo \emph{M} preguntando a cada nodo si se opone a la adquisión de tal bloqueo y si no, que lo agregue a su lista de locks. Si nadie se opone, el lock se adquiere y las listas de locks quedan actualizadas con este nuevo lock.
...
\subsubsection{Solución}
La solución fue crear un caché que guarda las respuestas.
Primero, se numeran los paquetes de cada worker. Así un paquete es identificado por el par (worker \emph{ID}, número de paquete).
Para cada paquete el caché guarda su respectiva respuesta. Ahora cuando se recibe un mensaje antes de procesarlo primero hay que fijarse en el caché si ya había sido  procesado anteriormente y si es así devolver la respuesta guardada en el caché.
